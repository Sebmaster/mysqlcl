...
RowPacket.convertBinary = function(packet, column, i) { // TODO: Implement more types.
	...
	switch (column.type) {
		...
		case 5: // DOUBLE
			...
			var signal = value[i.index] >> 7;
			var exponent = ((value[i.index++] & 127) << 4) + (value[i.index] >> 4);
			var significand = 0;
			var divisor = 2;
			do {
				var byteValue = value[i.index++];
			...
			return exponent == 2047 ? significand ? NaN : signal ? -Infinity : +Infinity
				: (1 + signal * -2) * (exponent || significand ? !exponent ? Math.pow(2, -1022) * significand
				: Math.pow(2, exponent - 1023) * (1 + significand) : 0);
			return;
		case 6: // NULL
			return null;
		case 7: // TIMESTAMP
