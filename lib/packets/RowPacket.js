...
RowPacket.convertBinary = function(packet, column, i) { // TODO: Implement more types.
	...
	switch (column.type) {
		...
		case 11: // TIME
			...
			return;
		case 12: // DATETIME
			var len = value[i.index++], // TODO: len? REALLY?
		...
		case 248: // SET
			...
			return;
		case 249: // TINYBLOB
			return value.slice(i.index, i.index += value[i.index++]); // MySQL 4.1?
		case 250: // MEDIUMBLOB
			return;
		case 251: // LONGBLOB
			return;
		case 252: // BLOB
			if (column.length === 255) {
				return value.slice(i.index, i.index += value[i.index++]);
			} else if (column.length === 765) {
				return value.slice(i.index, i.index += value[i.index++]).toString('utf8');
			}
		case 255: // GEOMETRY
			return;
	}
