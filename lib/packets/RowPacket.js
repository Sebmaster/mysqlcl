...
function RowPacket(packet, binary, autoBox) {
	...
	if (binary) {
		...
		for (var i=0; i < binary.length; ++i) {
			...
			this.data[i] = RowPacket.convertBinary(packet, binary[i], idx);
		}
	} else {
		if (autoBox) {
...
RowPacket.convertBinary = function(packet, column, i) { // TODO: Implement more types.
	...
	switch (column.type) { // TODO: Signed numbers not supported, yet
		case 0: // DECIMAL
			break;
		case 1: //TINY
		...
		case 10: // DATE
			...
			break;
		case 11: // TIME
			return new Date(value[i.index++] + (value[i.index++] << 8) + (value[i.index++] << 16)*1000);
			break;
		case 12: // DATETIME
			i.index += 8;
			break;
		...
		case 252: // BLOB
			...
			break;
		case 254: // STRING
			// TODO: The first byte is always zero! Second+x bytes are the length. How long is x?
			break;
		case 255: // GEOMETRY
			break;
	}
