...
Connection.prototype.data = function(data) {
	...
	for (var i=0; i < data.length;) {
		try {
			...
			this.packetQueue[packet.nr] = packet;
		} catch (e) {
	...
	for (var i=this.lastPacket; (packet = this.packetQueue[i]) instanceof Packet; ++i)  {
		switch (this.state) {
			case Connection.States.HANDSHAKE:
				var processedPacket = new HandshakePacket(packet);
				this.scramble = processedPacket.scramble;
				...
				switch (packet.data[0]) {
					...
					case 0xFF:
						...
						if (packet.data[0] === 0x00 && q.opcode === 0x16) {
							var query = new Query(q.cmd, this);
							query.prepareInfo = new PreparePacket(packet);
							if (q.listener) {
								q.listener(null, query);
							}
						} else {
							var processedPacket = new ParseablePacket(packet);
							if (q.listener) {
					...
					default:
						switch (q.state) {
							...
							case Connection.Fetchstate.ROW:
								...
								var processedPacket = new RowPacket(packet, packet.isBinaryRow ? q.fields : false, this.options.autoBox),
 ...
 * 
 * @param {function(string, ParseablePacket|Query)} cb a callback to call after query execution
 */
 ...
 */
Connection.prototype.execute = function(handle, flags, params, new_params, cb, columnCb, rowCb) {
	this.queue.push({opcode: 0x17, cmd: [handle, flags, params, new_params], fields: [], rows: [], listener: cb, columnCb: columnCb, rowCb: rowCb});
	if (this.queue.length === 1) {
		this.request();
	}
};
			...
			case 0x17:
				...
				if (params.length > 0) {
					...
					for (var i=0; i < params.length; i+=8) {
						for (var j=i+8; j > i; --j) {
							if (params[j] && params[j].value === null) {
								++num;
							}
							num <<= 1;
						}
