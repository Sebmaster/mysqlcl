 ...
 * @param {function(string, RowPacket)} rowCb a callback to call for each row/buffered
 */
Connection.prototype.listFields = function(table, column, cb, columnCb, rowCb) {
	...
	this.queue.push({opcode: 0x04, cmd: [table, column], state: Connection.Fetchstate.FIELD, listener: cb, columnCb: columnCb, rowCb: rowCb, fields: [], rows: []});
	if (this.queue.length === 1) {
		this.request();
	}
};
 ...
 */
Connection.prototype.refresh = function(type, cb, columnCb, rowCb) {
	this.queue.push({opcode: 0x07, cmd: type, state: undefined, listener: cb, columnCb: columnCb, rowCb: rowCb, fields: [], rows: []});
	if (this.queue.length === 1) {
		this.request();
	}
};
 ...
 * @param {function(string, ParseablePacket)} cb a callback to call after query execution
 */
Connection.prototype.shutdown = function(type, cb, columnCb, rowCb) {
	if (!type) {
		type = 0x00;
	}
	this.queue.push({opcode: 0x08, cmd: type, state: undefined, listener: cb, columnCb: columnCb, rowCb: rowCb, fields: [], rows: []});
	if (this.queue.length === 1) {
		this.request();
	}
};
