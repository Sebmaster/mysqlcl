var createConnection = require("net").createConnection,
	...
	HeaderPacket = require("./HeaderPacket").HeaderPacket,
	ColumnPacket = require("./ColumnPacket").ColumnPacket,
	RowPacket = require("./RowPacket").RowPacket;
 ...
 * 
 * @type {Array.<{opcode: number, cmd: string, listener: function(ParseablePacket), state: Connection.Fetchstate, fields: Array.<Packet>, rows: Array.<Packet>}>}
 */
			...
			case Connection.States.QUERYING:
				switch (packet.data[0]) {
					...
					default:
						...
						switch (q.state) {
							case undefined:
								q.state = Connection.Fetchstate.HEADER;
								var processedPacket = new HeaderPacket(packet);
								if (q.listener) {
									q.listener(null, processedPacket);
								}
							...
							case Connection.Fetchstate.ROW:
								if (packet.data[0] == 0xFE) {
									i += packet.length+4;
									q.state = Connection.Fetchstate.END;
									continue;
								}
